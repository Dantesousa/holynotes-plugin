...
import { deleteCacheFromDataStore, DeleteEntireStore, saveCacheToDataStore, PersonalPinsStore } from "./utils";

export const noteHandlerCache = new Map();

type NotebookChangeListener = () => void;
const notebookChangeListeners = new Set<NotebookChangeListener>();

export function subscribeNotebookChanges(listener: NotebookChangeListener) {
    notebookChangeListeners.add(listener);
    return () => notebookChangeListeners.delete(listener);
}

function notifyNotebookChange() {
    for (const listener of notebookChangeListeners) listener();
}

export default new (class NoteHandler {
    private _formatNote(channel: Channel, message: Message): HolyNotes.Note {
        const attachments = message.attachments?.map(att => {
            console.log("[PersonalPins] Attachment found:", {
                id: att.id,
        ...
        console.log("[PersonalPins] Formatting note:", {
    ...
    public addNote = async (message: Message, notebook: string) => {
        ...
        const debugInfo = {
            notebook,
            messageId: message.id,
            attachmentCount: formattedNote.attachments?.length ?? 0,
            attachments: formattedNote.attachments,
            contentLength: formattedNote.content?.length
        };
        
        // Debug: Mostrar no console e guardar info num atributo global
        console.warn("[PersonalPins] ADDING NOTE:", debugInfo);
        (window as any).__PERSONAL_PINS_DEBUG = debugInfo;
        
        const newNotes = Object.assign({ [message.id]: formattedNote }, notes);

        console.log("[PersonalPins] Saving notes:", newNotes);
    ...
    public importNotes = async (notes: HolyNotes.Note[]) => {
        ...
        for (const notebook in parseNotes) {
            ...
            saveCacheToDataStore(notebook, parseNotes[notebook]);
        }
