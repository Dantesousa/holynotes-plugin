...
import { ContextMenuApi, FluxDispatcher, Menu, React, Text, TextInput } from "@webpack/common";

import noteHandler, { noteHandlerCache, subscribeNotebookChanges } from "../../NoteHandler";
...
export const NoteModal = (props: ModalProps & { onClose: () => void; }) => {
    ...
    React.useEffect(() => {
        return subscribeNotebookChanges(() => forceUpdate());
    }, []);
    ...
    return (
        <ErrorBoundary>
            <ModalRoot {...props} className={classes("vc-notebook")} size={ModalSize.LARGE}>
                <div className={classes("vc-notebook-flex")} style={{ display: "flex", flexDirection: "column", width: "100%" }}>
