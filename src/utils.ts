...
export const PersonalPinsStore = createStore("PersonalPinsData", "PersonalPinsStore");

export async function saveCacheToDataStore(key: string, value?: Record<string, any>) {
    await DataStore.set(key, value, PersonalPinsStore);
}

export async function deleteCacheFromDataStore(key: string) {
    await DataStore.del(key, PersonalPinsStore);
}

export async function getFormatedEntries() {
    const data = await DataStore.entries(PersonalPinsStore);
...
export async function DeleteEntireStore() {
    await DataStore.clear(PersonalPinsStore);
